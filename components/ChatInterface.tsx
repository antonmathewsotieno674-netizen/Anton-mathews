  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      {messages.map((msg, index) => (
        ...
        >
          <div 
            className={`max-w-[85%] md:max-w-[70%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm transition-colors ${
              msg.role === 'user' 
          ...
          >
            {msg.role === 'model' && (
              <div className="font-semibold text-xs text-slate-400 dark:text-slate-500 mb-1 uppercase tracking-wider">MOA AI</div>
            )}
      ...
      {isLoading && (
        <div className="flex justify-start w-full">
          <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl rounded-tl-none px-4 py-3 shadow-sm flex items-center gap-2 transition-colors">
