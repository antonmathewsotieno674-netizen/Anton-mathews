...
const App: React.FC = () => {
  ...
  if (currentView === 'landing') {
    return <LandingPage onGetStarted={() => setCurrentView('auth')} onLogin={() => setCurrentView('auth')} theme={theme} toggleTheme={toggleTheme} />;
  }
        ...
        <header className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border-b border-white/50 dark:border-slate-700 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-sm transition-colors">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              title="Generate actionable tasks and export to Todoist/Google Calendar"
            >
            ...
            >
               <div className="w-8 h-8 rounded-full bg-brand-100 dark:bg-brand-900 text-brand-700 dark:text-brand-300 flex items-center justify-center font-bold text-xs border border-brand-200 dark:border-brand-800 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col md:flex-row max-w-7xl mx-auto w-full overflow-hidden">
          ...
          <div className="w-full md:w-80 bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border-r border-white/50 dark:border-slate-700 p-6 flex flex-col z-10 overflow-y-auto transition-colors">
            <div className="flex-1">
              ...
              <label id="upload-area" className={`flex flex-col items-center justify-center w-full min-h-[180px] border-2 border-dashed ${uploadSuccess ? 'border-green-500 bg-green-50 dark:bg-green-900/20 shadow-lg shadow-green-500/10' : isProcessingFile ? 'border-brand-400 bg-brand-50 dark:bg-brand-900/20' : 'border-slate-300 dark:border-slate-600 bg-white/50 dark:bg-slate-800/50 hover:bg-white/80 dark:hover:bg-slate-800/80 hover:border-brand-300 dark:hover:border-brand-500'} ${!file && !isProcessingFile && !uploadSuccess ? 'animate-pulse shadow-[0_0_15px_rgba(14,165,233,0.15)] border-brand-300 dark:border-brand-700' : ''} rounded-xl cursor-pointer transition-all group mb-4 shadow-sm relative overflow-hidden`}>
                <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center px-4 w-full">
                  {uploadSuccess ? (
                    ...
                    <>
                      {/* Visual Cues for File Types */}
                      <div className="flex gap-4 mb-4 opacity-70 group-hover:opacity-100 transition-opacity">
                        <div className="flex flex-col items-center gap-1">
                          <div className="w-10 h-10 bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 rounded-lg flex items-center justify-center shadow-sm">
                            <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9v-2h2v2zm0-5h2v3h-2V7z"/></svg>
                          </div>
                          <span className="text-[10px] font-bold text-slate-400">PDF</span>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                           <div className="w-10 h-10 bg-blue-50 dark:bg-blue-900/20 text-blue-500 dark:text-blue-400 rounded-lg flex items-center justify-center shadow-sm">
                             <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm2 14h-3v3h-2v-3H8v-2h3v-3h2v3h3v2zm-3-7V3.5L18.5 9H13z"/></svg>
                           </div>
                           <span className="text-[10px] font-bold text-slate-400">DOCX</span>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                           <div className="w-10 h-10 bg-purple-50 dark:bg-purple-900/20 text-purple-500 dark:text-purple-400 rounded-lg flex items-center justify-center shadow-sm">
                             <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M21 19V5a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg>
                           </div>
              ...
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 mb-2 py-2.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 rounded-xl transition-all font-medium text-sm border border-slate-200 dark:border-slate-600"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div id="chat-input-area" className="p-4 md:p-6 pb-6 border-t border-slate-100 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 backdrop-blur-md relative z-10 transition-colors">
              ...
              <div className="text-center mt-2 flex justify-center items-center gap-2">
                 <p className="text-[10px] text-slate-400 dark:text-slate-500">
                    MOA AI can make mistakes. Verify important information. 
                 </p>
                 {!userState.isPremium && remainingQuestions !== null && (
                    <span className="text-[10px] text-brand-600 dark:text-brand-400 font-medium bg-brand-50 dark:bg-brand-900/20 px-1.5 py-0.5 rounded">
